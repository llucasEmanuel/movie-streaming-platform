...
async function main() {
  ...
  await prisma.history.create({
    data: { userId: usuarioJulio.id, movieId: v1.id, watchedAt: new Date() }
  });

  // Júlio assistiu 3 filmes de Terror recentemente (Gera a playlist "Recomendações de Terror")
  await prisma.history.create({
    data: { userId: usuarioJulio.id, movieId: t1.id, watchedAt: new Date() }
  });
  await prisma.history.create({
    data: { userId: usuarioJulio.id, movieId: v2.id, watchedAt: new Date(Date.now() - 24 * 60 * 60 * 1000) } // Ontem
  });
